...
export default function EditarFAS() {
  const { id } = useParams();
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await base44.entities.FAS.update(id, form);
    setSaving(false);
    navigate(`/fas/${id}`);
  };
  ...
  if (!canEdit) {
    return (
      <div className="p-6 max-w-xl mx-auto text-center mt-20">
        <p className="text-muted-foreground text-lg">Sem permissão para editar FAS.</p>
        <Button variant="outline" className="mt-4" onClick={() => navigate(`/fas/${id}`)}>Voltar</Button>
      </div>
    );
  }

  if (!form) {
    return <div className="p-6 text-center text-muted-foreground">FAS não encontrada.</div>;
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate(`/fas/${id}`)}>
          <ArrowLeft className="w-4 h-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Editar FAS</h1>
          <p className="text-sm text-muted-foreground font-mono-data mt-0.5">{form.numero_fas || form.numero_proposta}</p>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 pb-4">
        <Button variant="outline" onClick={() => navigate(`/fas/${id}`)}>Cancelar</Button>
        <Button
          onClick={handleSave}
