...
export default function NovoRecebimento({ open, onClose, onSaved, totalRecebimentos }) {
  ...
  const handleAddAmostra = () => {
    ...
    const amostraComId = { ...amostraTemp, id: Math.random().toString(36).substr(2, 9) };
    setForm(f => ({ ...f, amostras: [...f.amostras, amostraComId] }));
    setAmostraTemp({
      material_id: '',
      material_nome: '',
      procedencia: '',
  ...
  const handleRemoveAmostra = (idx) => {
    setForm(f => ({ ...f, amostras: f.amostras.filter((_, i) => i !== idx) }));
  };

  const handleSalvar = async () => {
    if (!form.cliente_id || !form.data_entrada) return;
    setLoading(true);
    const numeroProtocolo = gerarNumeroProtocolo(totalRecebimentos);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <Label className="text-xs">Finalidade (Tipo Ensaio)</Label>
              <Input value={form.finalidade} onChange={e => setForm(f => ({ ...f, finalidade: e.target.value }))} className="mt-1" placeholder="Ex: Análise de Solos" />
            </div>
            <div>
              <Label className="text-xs">Nº FAS</Label>
              <Input value={form.numero_fas} onChange={e => setForm(f => ({ ...f, numero_fas: e.target.value }))} className="mt-1" placeholder="Ex: FAS-001" />
            </div>
            <div>
              ...
              <CardContent className="p-3">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
                  ...
                  <div>
                    ...
                    <Input type="number" value={amostraTemp.quantidade} onChange={e => setAmostraTemp(a => ({ ...a, quantidade: e.target.value }))} className="mt-1 h-8" placeholder="Qtd" />
                  </div>
                  <div>
                    <Label className="text-xs">Observação Recebimento</Label>
                    <Input value={amostraTemp.observacao_recebimento} onChange={e => setAmostraTemp(a => ({ ...a, observacao_recebimento: e.target.value }))} className="mt-1 h-8" placeholder="Obs" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input type="number" step="0.01" value={amostraTemp.peso_kg} onChange={e => setAmostraTemp(a => ({ ...a, peso_kg: e.target.value }))} className="mt-1 h-8" placeholder="Peso" />
                  </div>
                  ...
                  <div className="flex items-end">
                    <Button onClick={handleAddAmostra} size="sm" variant="outline" className="gap-1 h-8 w-full text-xs">
                  ...
                  <tbody className="divide-y divide-border">
                    {form.amostras.map((a, idx) => (
                      <tr key={idx} className="hover:bg-muted/20">
                        ...
                        <td className="px-2 py-1 text-xs">{a.observacao_recebimento || '—'}</td>
                        <td className="px-2 py-1 text-xs">{a.data_coleta ? new Date(a.data_coleta).toLocaleDateString('pt-BR') : '—'}</td>
                        <td className="px-2 py-1 text-right">{a.peso_kg || '—'}</td>
                        <td className="px-2 py-1 text-center">{a.quantidade_suficiente ? 'S' : 'N'}</td>
                        <td className="px-2 py-1 text-center">
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleRemoveAmostra(idx)}>
